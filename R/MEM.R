...
############## Funcao Log-verossimilhanca a ser maximizada

### Caso 1: lambda_e conhecido (irrestrito)
log.ver.lambda.e <- function(theta, y, x, lambda_e)
{
  beta      = theta[1]
  mu_x      = theta[2]
  alfa      = theta[3]
  ...
  #### Vetor de médias
  mu = matrix(c((alfa + beta*mu_x), mu_x), ncol=1, nrow=2)
  
  ##### Matrix de Covariancias
  Sigma = matrix(c((lambda_e*sigma2_u) + (beta^2*sigma2_x),
                   beta*sigma2_x,
                   beta*sigma2_x,
                   sigma2_x + sigma2_u), ncol=2,nrow=2)
 
  Inv_Sigma = solve(Sigma)    
  Det_Sigma = det(Sigma)  
  
  Z          = array(NA,c(2,1,n))
  diferenca  = array(NA,c(2,1,n))
  Produto    = c()
  for (i in 1:n)
  {
    Z[,,i]         = matrix(c(y[i],x[i]),ncol = 1,nrow = 2)      
    diferenca[,,i] = matrix(Z[,,i] - mu[,],ncol = 1)
    Produto[i]     = (t(matrix(diferenca[,,i],ncol = 1)) %*% Inv_Sigma %*% matrix(diferenca[,,i],ncol = 1))
    
  }      
  
  ##### Log da verossimilhanca irrestrita
  return(- (n/2)*log(Det_Sigma) - (1/2)*sum(Produto))
  
}

### Caso 1: lambda_e conhecido (restrito a H0)
log.ver.lambda.e.H0 <- function(theta, y, x, lambda_e, beta_til)
{
  mu_x      = theta[1]
  alfa      = theta[2]
  sigma2_u  = theta[3]
  ...
  #### Vetor de médias
  mu = matrix(c((alfa + beta_til*mu_x), mu_x), ncol=1, nrow=2)
  
  ##### Matrix de Covariancias
  Sigma = matrix(c(lambda_e*sigma2_u+(beta_til^2*sigma2_x),
                      beta_til*sigma2_x,
                      beta_til*sigma2_x,
                      sigma2_x + sigma2_u), ncol=2,nrow=2)
  ...
  Z          = array(NA,c(2,1,n))
  diferenca  = array(NA,c(2,1,n))
  Produto    = matrix(NA,ncol = 1,nrow = 2)
  for (i in 1:n)
  {
    Z[,,i]         = matrix(c(y[i],x[i]),ncol = 1,nrow = 2)      
    diferenca[,,i] = matrix(Z[,,i] - mu,ncol = 1)
...
### Caso 2: lambda_x conhecido (irrestrito)
log.ver.lambda.x <- function(theta, y, x, lambda_x)
{
  beta      = theta[1]
  mu_x      = theta[2]
  alfa      = theta[3]
  ...
  ##### Matrix de Covariancias
  Sigma = matrix(c(lambda_x*beta^2*sigma2_u + sigma2_e,
                   lambda_x*beta*sigma2_u,
                   lambda_x*beta*sigma2_u,
                   (lambda_x + 1)*sigma2_u), ncol=2,nrow=2)
...
### Caso 2: lambda_x conhecido (restrito a H0)
log.ver.lambda.x.H0 <- function(theta, y, x, lambda_x, beta_til)
{
  mu_x      = theta[1]
  alfa      = theta[2]
  sigma2_u  = theta[3]
  ...
  ##### Matrix de Covariancias
  Sigma = matrix(c(lambda_x*beta_til^2*sigma2_u + sigma2_e,
                   lambda_x*beta_til*sigma2_u,
                   lambda_x*beta_til*sigma2_u,
                   (lambda_x + 1)*sigma2_u), ncol=2,nrow=2)
  ...
  if(is.vector(x) & is.vector(y) & is.numeric(x) & is.numeric(y) & (length(x) == length(y)) &
    ...
    quantil_90 = qchisq(0.90,1)
    quantil_95 = qchisq(0.95,1)
    quantil_99 = qchisq(0.99,1)	 
    ...
    if(lambda_x == F & lambda_e > 0){
      ...
      ## Valores iniciais irrestritos
      beta_inic     = (S_yy - (S_xx*lambda_e) + ( (S_yy - (S_xx*lambda_e))^(2) + 4*lambda_e*(S_xy^(2)) )^(1/2) )/(2*S_xy)
      mu_x_inic     = x_barra
      alfa_inic     = y_barra - (beta_inic*x_barra)
      sigma2_u_inic = ( S_yy - 2*beta_inic*S_xy + beta_inic*beta_inic*S_xx )/( beta_inic*beta_inic + lambda_e )
      sigma2_x_inic = S_xy/beta_inic
      
      ## Valores iniciais restritos sob H0
      alfa_inic.H0     = y_barra - (beta_til*x_barra)
      mu_x_inic.H0     = ( y_barra*beta_til + lambda_e*x_barra - alfa_inic.H0*beta_til )/( lambda_e + beta_til*beta_til)
      sigma2_u_inic.H0 = ( S_yy - 2*beta_til*S_xy + beta_til*beta_til*S_xx + (x_barra*beta_til - y_barra + alfa_inic.H0 )^2)/( beta_til*beta_til + lambda_e)
      sigma2_x_inic.H0 = ( S_yy*(beta_til*beta_til - lambda_e) + 4*beta_til*lambda_e*S_xy - (S_xx*lambda_e*(beta_til*beta_til - lambda_e)) - lambda_e*((x_barra*beta_til + alfa_inic.H0)^2) + y_barra*lambda_e*(2*alfa_inic.H0 -y_barra+(2*x_barra*beta_til)))/( (beta_til*beta_til+lambda_e)^2) 
      
      # Vetor de valores iniciais
      theta.inic    = c(beta_inic,mu_x_inic,alfa_inic,sigma2_u_inic,sigma2_x_inic)
      theta.inic.H0 = c(mu_x_inic.H0,alfa_inic.H0,sigma2_u_inic.H0,sigma2_x_inic.H0)
      
      
      ## Maximizando a log-verossimilhanca
      Estimacao = optim(theta.inic, log.ver.lambda.e, y = y, x = x, lambda_e = lambda_e, NULL,method = "BFGS", control=list(fnscale=-1), hessian=T)
      
      ##Estimativas de maxima verossimilhanca (e.m.v.)
      beta_hat     = Estimacao$par[1]
      mu_x_hat     = Estimacao$par[2]
      alfa_hat     = Estimacao$par[3]
      sigma2_u_hat = Estimacao$par[4]
      sigma2_x_hat = Estimacao$par[5]
      
      ## Maximizando a log-verossimilhanca
      Estimacao.H0 = optim(theta.inic.H0, log.ver.lambda.e.H0, y = y, x = x, lambda_e = lambda_e, beta_til = beta_til, NULL,method = "BFGS", control=list(fnscale=-1), hessian=T)
      
      ##Estimativas de maxima verossimilhanca (e.m.v.)
      mu_x_til     = Estimacao.H0$par[1]
      alfa_til     = Estimacao.H0$par[2]
      sigma2_u_til = Estimacao.H0$par[3]
      sigma2_x_til = Estimacao.H0$par[4]
      ...
      ##Estimativas de maxima verossimilhanca (e.m.v.)
      beta_inic     = ((lambda_x + 1)*S_xy)/(lambda_x*S_xx)
      mu_x_inic     = x_barra
      alfa_inic     = y_barra - (beta_inic*x_barra)
      sigma2_u_inic = S_xx/(lambda_x + 1)
      sigma2_e_inic = (lambda_x*(S_yy*S_xx - S_xy^2) - S_xy^2)/(lambda_x*S_xx)
      
      ##Estimativas de maxima verossimilhanca (e.m.v.) sob H0
      alfa_inic.H0     = y_barra - (beta_til *x_barra)
      mu_x_inic.H0     = x_barra;
      sigma2_u_inic.H0 = S_xx/(lambda_x + 1);
      sigma2_e_inic.H0 = ((lambda_x-1)*lambda_x*beta_til^2*S_xx - 2*(lambda_x+1)*lambda_x*beta_til*S_xy +	(lambda_x+1)^2*S_yy)/((lambda_x+1)^2)
      
      
      # Vetor de valores iniciais
      theta.inic    = c(beta_inic,mu_x_inic,alfa_inic,sigma2_u_inic,sigma2_e_inic)+2
      theta.inic.H0 = c(mu_x_inic.H0,alfa_inic.H0,sigma2_u_inic.H0,sigma2_e_inic.H0)+2
      
      ## Maximizando a log-verossimilhanca
      Estimacao = optim(theta.inic, log.ver.lambda.x, y = y, x = x, lambda_x = lambda_x, NULL,method = "BFGS", control=list(fnscale=-1), hessian=T)
      ...
      alfa_hat     = Estimacao$par[3]
      sigma2_u_hat = Estimacao$par[4]
      sigma2_e_hat = Estimacao$par[5]
      
      ## Maximizando a log-verossimilhanca
      Estimacao.H0 = optim(theta.inic.H0, log.ver.lambda.x.H0, y = y, x = x, lambda_x = lambda_x, beta_til = beta_til, NULL, method = "BFGS", control=list(fnscale=-1), hessian=T)
      
      ##Estimativas de maxima verossimilhanca (e.m.v.)
      mu_x_til     = Estimacao.H0$par[1]
      alfa_til     = Estimacao.H0$par[2]
      sigma2_u_til = Estimacao.H0$par[3]
      sigma2_e_til = Estimacao.H0$par[4]
    ...
    if(Correction){ # Segunda parte do correction TRUE
      ...
      ##### Valor de p
      VP_LR_B1 = 1 - pchisq(LR_B1, p )
      VP_LR_B2 = 1 - pchisq(LR_B2, p )
      VP_LR_B3 = 1 - pchisq(LR_B3, p )  
    ...
    if(Correction){ 
      ...
      if(lambda_e == F & lambda_x > 0){ # Caso lamda_x
        ...
        Saida = function(){
          cat("\n")
          cat("Call: \n")
          cat("MEM( formula = y ~ x, = lambda_x = ", lambda_x, " Correction = FALSE)" ," \n")
          cat("\n")
          cat("Coefficients: \n")
          print.data.frame(Coefficients,row.names = F)
          cat("--- \n")
        ...
        Saida = function(){
          cat("\n")
          cat("Call: \n")
          cat("MEM( formula = y ~ x, = lambda_e = ", lambda_e, " Correction = FALSE)" ," \n")
          cat("\n")
          cat("Coefficients: \n")
          print.data.frame(Coefficients,row.names = F)
          cat("--- \n")
